...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto createItem(CreateItemRequest request, Long userId) {
        ...
        if (!request.isRequestIdEmpty()) {
            ...
            itemToCreate = ItemMapper.mapToEntity(request, user, itemRequest);
        } else {
            itemToCreate = ItemMapper.mapToEntity(request, user);
        }
    ...
    public ItemDto updateItem(UpdateItemRequest request, Long userId, Long itemId) {
        ...
        Item oldItemData = itemRepository.findById(itemId)
        ...
        if (request.isAvailableEmpty()) {
            request.setAvailable(oldItemData.getIsAvailable());
        }
        ...
        if (request.isRequestIdEmpty()) {
            if (oldItemData.getRequest() != null) {
                itemToUpdate = ItemMapper.mapToEntity(request, user, itemId, oldItemData.getRequest());
            } else {
                itemToUpdate = ItemMapper.mapToEntity(request, user, itemId);
            }
        } else {
            ItemRequest itemRequest = itemRequestRepository.findById(request.getRequestId())
                    .orElseThrow(() -> new NotFoundException("Item request not found"));
            itemToUpdate = ItemMapper.mapToEntity(request, user, itemId, itemRequest);
        }
